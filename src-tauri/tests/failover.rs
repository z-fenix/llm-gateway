...
async fn assert_failover_to_secondary(primary_status: u16, primary_body: serde_json::Value) {
    ...
    let res = forward(&state, &chat(), None, &api_key(&repo))
        .await
        .unwrap();
    assert_eq!(res.outcome.channel.id, "secondary");
    assert!(
        !secondary_mock.hits.lock().unwrap().is_empty(),
        "secondary should be hit"
    );

    let primary_after = repo.get_channel("primary").unwrap().unwrap();
    assert!(
        primary_after.success_rate < 1.0,
        "primary success_rate should drop after failover"
    );
    assert!(
        primary_mock.hits.lock().unwrap().len() == 1,
        "primary should be hit once"
    );
}
...
async fn dispatch_primary_network_unreachable_falls_back() {
    ...
    assert!(
        primary_after.success_rate < 1.0,
        "primary success_rate should drop after network failure"
    );
}
...
async fn dispatch_all_candidates_fail_returns_5xx() {
    let (primary_base, _) =
        common::spawn_mock(500, serde_json::json!({"error":"primary boom"})).await;
    let (secondary_base, _) =
        common::spawn_mock(503, serde_json::json!({"error":"secondary down"})).await;
    ...
    assert!(
        !trace_id.is_empty(),
        "response must include a non-empty trace_id"
    );
    ...
    assert_eq!(
        ...
        "request log trace_id must match response trace_id"
    );
    ...
    assert!(
        primary_after.success_rate < 1.0,
        "primary success_rate should drop"
    );
    assert!(
        secondary_after.success_rate < 1.0,
        "secondary success_rate should drop"
    );
}

#[tokio::test]
async fn dispatch_primary_400_does_not_fallback() {
    let (primary_base, primary_mock) =
        common::spawn_mock(400, serde_json::json!({"error":"bad request"})).await;
    let (secondary_base, secondary_mock) = common::spawn_mock(200, ok_openai_body()).await;
    ...
    let err = forward(&state, &chat(), None, &api_key(&repo))
        .await
        .unwrap_err();
    match err {
    ...
    assert!(
        secondary_mock.hits.lock().unwrap().is_empty(),
        "secondary should NOT be hit for non-failover 4xx"
    );

    let primary_after = repo.get_channel("primary").unwrap().unwrap();
    assert!(
        primary_after.success_rate < 1.0,
        "primary success_rate should drop after 400"
    );
}
