...
async fn responses_stream_sse_e2e() {
    ...
    let created = body.find("response.created").expect("missing response.created");
    let completed = body.find("response.completed").expect("missing response.completed");
    assert!(created < completed, "response.created should appear before response.completed");
