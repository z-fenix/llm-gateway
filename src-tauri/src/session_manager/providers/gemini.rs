...
pub fn load_messages(path: &Path) -> Result<Vec<SessionMessage>, String> {
    ...
    for msg in messages {
        ...
        // Gemini content 可能是字符串或 {text: ...} 数组
...
mod tests {
    ...
    fn delete_session_removes_json_file() {
        ...
        let path = dir.path().join("session-2026-03-06T10-17-test.json");
        std::fs::write(
            &path,
            r#"{
        ...
        delete_session(dir.path(), &path, "gemini-session-123").unwrap();
        assert!(!path.exists());
    }
    ...
    #[test]
    fn parse_session_extracts_title_from_first_user_message() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("session.json");
        std::fs::write(
            &path,
            r#"{
              ...
              "lastUpdated": "2026-03-06T10:20:00.000Z",
              "messages": [
                {"type":"user","content":"Ask about the weather"},
        ...
        assert_eq!(meta.title.as_deref(), Some("Ask about the weather"));
