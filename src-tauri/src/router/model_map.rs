...
mod tests {
    ...
    fn resolve_hit_and_miss() {
        let maps = vec![("gpt-4o".to_string(), "gpt-4o-2024-08-06".to_string())];
    ...
    fn model_map_crud_and_resolve() {
        ...
        repo.set_model_map("c1", "claude-sonnet-4", "deepseek-v4-flash")
            .unwrap();
        // 覆盖更新
        repo.set_model_map("c1", "claude-sonnet-4", "deepseek-v4-flash-0715")
            .unwrap();
        let maps = repo.get_model_map("c1").unwrap();
        assert_eq!(maps.len(), 1);
        assert_eq!(
            resolve_model(&maps, "claude-sonnet-4"),
            "deepseek-v4-flash-0715"
        );
    }
}
